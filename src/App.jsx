...
const RACES={
  ...
  Äffling:{bw:5,st:1,ge:5,in_:2,hp:4,label:"Äffling",emoji:"🐒",desc:"Takelage-Akrobat"},
...
const PROFS={
  ...
  Kanonier:{label:"Kanonier",nk:0,fk:2,emoji:"💣",desc:"Fernkampf",skills:["Breitseite","Kettenschuss","Feuerkanone","Präzisionsschuss","Inferno"]},
  Schiffsarzt:{label:"Schiffsarzt",nk:0,fk:0,emoji:"⚕️",desc:"Heilt Crew",skills:["Wundversorgung","Kräuterkunde","Feldchirurg","Giftkunde","Wiederbelebung"]},
  VoodooPriester:{label:"Voodoo",nk:0,fk:0,emoji:"🔮",desc:"Dunkle Magie",skills:["Verfluchung","Geisterruf","Schutzamulett","Seelenraub","Totenbeschwörung"]},
};
const SHIPS=[
...
const SHOP_INV={
  ...
  dorf:{name:"Fischerhändler",emoji:"🏘️",ids:["dolch","enterhaken","lederwams","proviant","rum","seil","fackel","heiltrank","buckler"],pm:1.15,ships:false},
  stadt:{name:"Goldküsten-Basar",emoji:"🏰",ids:["rapier","axt","flamberg","doppellauf","dreizack","kette","brust","buckler","rum","heiltrank","elixier","kugeln","dynamit","fernrohr","kompass","voodoo","amulett","seekarte","geisterflasche","neptunklinge","donnerbuechse","drachenschuppe"],pm:0.9,ships:true},
  festung:{name:"Militärvorräte",emoji:"⚔️",ids:["rapier","axt","flamberg","muskete","doppellauf","kette","brust","buckler","kugeln","dynamit","neptunklinge","donnerbuechse","drachenschuppe"],pm:0.75,ships:false},
};
...
const REGIONS=[
  ...
  {id:"goldkueste",name:"Goldküste",type:"stadt",x:78,y:58,minF:10,conn:["handelsweg","festung","haifischbucht"],shop:"stadt",port:true,tavern:true},
...
// ═══════════════════════════════════════════════════════════
const PN=["Einauge","Rotbart","Schwarzzahn","La Muerte","Knochenbrecher","Silberfinger","Sturmwind","Bluthund","Donnerschlag","Goldkralle","Schlangenauge","Nebelfaust","Totenkopf-Tom","Haifisch-Henri","Eisenbart","Krakenjäger","Dunkle Dolores","Teufelszunge","Galgenvogel","Rum-Rosita","Feuerfaust","Giftzahn","Wirbelwind-Wanda","Voodoo-Vic","Krummsäbel-Karl","Narbengesicht","Sturmbraut","Eisenhaken","Goldzahn-Gustav","Mondauge","Taifun-Tessa","Messerhans","Kanonen-Klaus","Korallen-König","Seemine-Sam","Ankerfaust","Schwarze Witwe","Schädelbrecher","Vulkan-Vera","Dreizack-Dimitri","Seeteufel","Nebelkrähe","Donnergroll","Kielholer","Nordwind-Nils","Salzblut","Riffbrecher","Sturmfalke","Breitseite-Bernd","Flaschenteufel","Brandungshexe","Wellenbrecher","Nebelhorn","Schatzgräber","Korallenherz","Meuterer-Moritz","Barrakuda-Bella","Leuchtqualle","Salzfinger","Wellentänzerin","Seemannsgarn","Reling-Rosa"];
const SN=["Sturmkrähe","Schwarze Perle","Blutige Mary","Seewolf","Nebeltänzerin","Todesschwinge","Goldener Hai","Meerjungfrau","Krakenzorn","Geisterschiff","Feuersbrunst","Sturmbrecher","Mondschatten","Wellenreiter","Korallendolch","Phantomklinge","Teufelsklaue","Piranhabiss","Leuchtfeuer","Ankerschreck","Voodoo-Queen","Salzwind","Neptuns Zorn","Galgenhumor","Schattensegel","Höllenfeuer","Eisenkiel","Sturmvogel","Kanonendonner","Nachtfalke","Blutrausch","Silberpfeil","Haifischzahn","Donnerkeil","Totenstille","Barrakuda","Perlentaucher","Bugwelle","Seemannsbraut"];
const CN=["Riesenkrake","Seeschlange","Geisterpiraten","Untote Matrosen","Riffhaie","Sumpfkrokodil","Giftige Quallen","Sirenen","Wasserelementar","Skelettcrew","Seehexe","Seeteufel","Piranha-Schwarm","Zombiepiraten","Voodoo-Golem","Sturmgeist","Nebeldämon","Korallenwächter","Lava-Krabbe","Vulkandrache","Bermuda-Phantom","Tentakelhorror","Flutwurm","Muschelgolem","Gezeitenbestie","Feuerfisch","Hai-König","Todesrochen","Barrakuda-Rudel","Hammerhai","Tiefseekrake","Leviathan-Junges"];
const ADJ=["verfluchte","goldene","vergessene","gespenstische","uralte","verrostete","leuchtende","mysteriöse","finstere","verborgene","verdammte","zerbrochene","legendäre","verschollene","geheime","stürmische","tosende","neblige","dunkle","brennende","eisige","giftige","schimmernde","blutige","silberne","kristallene","schwarze","smaragdgrüne","pechschwarze","mondbeleuchtete","tödliche","verzauberte"];
const TN=["Aztekengold","Rubinkrone","Smaragdkelch","Neptuns Dreizack","Sirenenharfe","Krakens Herz","Davy Jones Schlüssel","Poseidons Gürtel","Schwarzer Opal","Blutrubin","Mondperle","Vulkanjuwel","Korallendiadem","Gezeitenring","Meerjungfrauenträne","Piratenkönigssiegel","Kristallschädel","Flammenherz","Diamantsäbel","Obsidianmaske","Phönixfeder","Leviathans Schuppe","Weltenkompass","Schicksalswürfel"];
...
const E={
flach:[d=>({text:ft("{adj} Handelsschiff der {sn} kreuzt euren Kurs."),type:"choice",opts:["Handeln","Überfallen","Vorbei"],rews:[{gold:3+roll(5),ruhm:1},{combat:mkE("Handelswachen",d),reward:{gold:10+roll(8),ruhm:2}},{}]}),d=>({text:ft("Treibgut der {sn}! Fässer mit {cargo}!"),type:"loot",gold:3+roll(6),ruhm:1}),d=>({text:ft("{cn} tauchen auf!"),type:"combat",enemy:mkE(ft("{cn}"),d),reward:{gold:roll(5),ruhm:2}}),d=>({text:ft("Fischer bieten Fang an."),type:"heal",amount:2+roll(2)}),d=>({text:ft("Flaschenpost! {adj} Schatzkarte von {pn}!"),type:"loot",gold:2+roll(4),ruhm:3}),d=>({text:ft("{adj} Kauffahrteischiff mit {cargo} in Seenot!"),type:"choice",opts:["Retten","Plündern","Ignorieren"],rews:[{ruhm:4,fame:1,gold:roll(5)},{gold:10+roll(8),ruhm:-1},{}]}),d=>({text:ft("Delfine! Moral steigt!"),type:"loot",gold:1+roll(3),ruhm:1,rum:2}),d=>({text:ft("Marine-Kutter! Schnell reagieren!"),type:"skilltest",stat:"ge",diff:4+d,pass:{ruhm:3},fail:{combat:mkE("Marine",d+1),reward:{gold:12,ruhm:3,fame:1}}}),d=>({text:ft("{pn} fordert zum Würfelduell!"),type:"choice",opts:["Annehmen","Nein"],rews:[Math.random()>.5?{gold:8,ruhm:2}:{gold:-5},{}]}),d=>({text:ft("Ausguck: {adj} Insel!"),type:"skilltest",stat:"in_",diff:3+d,pass:{gold:6+roll(8),ruhm:3,fame:1},fail:{ruhm:1}}),d=>({text:ft("Sturm! Navigator muss steuern!"),type:"skilltest",stat:"ge",diff:5+d,pass:{ruhm:3},fail:{gold:-3}}),d=>({text:ft("Ruhige See — eure Crew fischt und findet dabei Gold im Bauch eines Fisches!"),type:"loot",gold:2+roll(4),ruhm:1}),d=>({text:ft("Alter Matrose auf Fass: kennt {tn}! Er zeichnet euch eine Karte."),type:"loot",gold:3+roll(3),ruhm:4,fame:1}),d=>({text:ft("{adj} Handelsposten auf kleiner Insel."),type:"trade",gold:roll(5)})],
handel:[d=>({text:ft("Galeone {sn} mit {cargo}!"),type:"choice",opts:["Überfallen","Handeln","Lassen"],rews:[{combat:mkE("Galeonen-Wachen",d+1),reward:{gold:15+roll(15),ruhm:3,fame:1}},{gold:5+roll(5)},{}]}),d=>({text:ft("Konvoi! Nachtangriff?"),type:"skilltest",stat:"ge",diff:5+d,pass:{gold:20+roll(10),ruhm:4,fame:1},fail:{combat:mkE("Konvoi-Eskorte",d+2),reward:{gold:25+roll(15),ruhm:5,fame:1}}}),d=>({text:ft("Schmuggler {pn} bietet {cargo}."),type:"skilltest",stat:"in_",diff:4+d,pass:{gold:5,ruhm:2},fail:{gold:-8}}),d=>({text:ft("Sinkender Frachter! {cargo}!"),type:"loot",gold:6+roll(10),ruhm:2}),d=>({text:ft("Marine-Fregatte {sn}! Kampf!"),type:"combat",enemy:mkE("Marine-Fregatte",d+2),reward:{gold:15+roll(10),ruhm:4,fame:1}}),d=>({text:ft("Passagierschiff!"),type:"choice",opts:["Überfallen","Lassen"],rews:[{combat:mkE("Leibwächter",d),reward:{gold:20+roll(10),ruhm:2,fame:1}},{ruhm:1}]}),d=>({text:ft("Treibende Fässer mit {cargo}."),type:"loot",gold:4+roll(6),ruhm:1})],
sumpf:[d=>({text:ft("{adj} Krokodil!"),type:"combat",enemy:mkE("Sumpfkrokodil",d),reward:{gold:roll(4),ruhm:2}}),d=>({text:ft("Giftnebel! Stärke!"),type:"skilltest",stat:"st",diff:4+d,pass:{ruhm:3},fail:{dmgAll:1}}),d=>({text:ft("Voodoo-Priesterin {pn}."),type:"choice",opts:["Besuchen","Meiden"],rews:[Math.random()>.4?{ruhm:4,fame:1,heal:3,removeCurse:true}:{curse:true},{}]}),d=>({text:ft("Versunkenes Piratenlager!"),type:"loot",gold:6+roll(8),ruhm:2}),d=>({text:ft("{cn} aus dem Morast!"),type:"combat",enemy:mkE(ft("{cn}"),d+1),reward:{gold:roll(6),ruhm:3,fame:1}}),d=>({text:ft("Geheimer Wasserweg!"),type:"skilltest",stat:"in_",diff:4+d,pass:{ruhm:4,fame:1},fail:{ruhm:0}})],
riff:[d=>({text:ft("Schiff auf {adj} Riff!"),type:"skilltest",stat:"ge",diff:4+d,pass:{ruhm:2},fail:{gold:-5}}),d=>({text:ft("Gold unter Wasser!"),type:"skilltest",stat:"ge",diff:3+d,pass:{gold:8+roll(10),ruhm:2},fail:{dmgAll:1}}),d=>({text:ft("{cn} am Riff!"),type:"combat",enemy:mkE(ft("{cn}"),d),reward:{gold:roll(6),ruhm:3}}),d=>({text:ft("Wrack der {sn}!"),type:"choice",opts:["Erkunden","Weiter"],rews:[{gold:10+roll(8),ruhm:4,fame:1},{}]}),d=>({text:ft("Perlenmuscheln!"),type:"loot",gold:5+roll(8),ruhm:2}),d=>({text:ft("{adj} Unterwasserhöhle!"),type:"choice",opts:["Tauchen","Nein"],rews:[Math.random()>.5?{gold:15+roll(10),ruhm:5,fame:1}:{combat:mkE("Höhlenwächter",d+2),reward:{gold:20,ruhm:4,fame:1}},{}]})],
hai:[d=>({text:ft("Weisser Hai!"),type:"combat",enemy:mkE("Weisser Hai",d+1),reward:{gold:roll(4),ruhm:4,fame:1}}),d=>({text:ft("Haifisch-Arena! {pn} laed ein!"),type:"choice",opts:["Wetten(5G)","Kämpfen","Nein"],rews:[Math.random()>.5?{gold:10,ruhm:2}:{gold:-5},{combat:mkE("Arena-Hai",d+1),reward:{gold:15,ruhm:5,fame:1}},{}]}),d=>({text:ft("Hai-Krieger von {pn}!"),type:"choice",opts:["Verhandeln","Angreifen"],rews:[{ruhm:2},{combat:mkE("Hai-Krieger",d+1),reward:{gold:10,ruhm:4,fame:1}}]}),d=>({text:ft("Im Haimagen: {adj} Schwert!"),type:"loot",gold:8+roll(6),ruhm:3}),d=>({text:ft("Hammerhaie! Mut!"),type:"skilltest",stat:"st",diff:5+d,pass:{ruhm:4,fame:1},fail:{dmgAll:2}})],
geister:[d=>({text:ft("Geisterschiff {sn}!"),type:"combat",enemy:mkE("Geisterpiraten",d+2),reward:{gold:12+roll(10),ruhm:5,fame:1}}),d=>({text:ft("Geist von {pn}!"),type:"choice",opts:["Befragen","Fliehen","Voodoo"],rews:[{ruhm:5,fame:1},{},{ruhm:6,fame:1,gold:roll(8)}]}),d=>({text:ft("{adj} Friedhof."),type:"choice",opts:["Gräber oeffnen","Respekt"],rews:[Math.random()>.5?{gold:15+roll(10),ruhm:3,fame:1}:{combat:mkE("Untote",d+2),reward:{gold:10,ruhm:4,fame:1}},{ruhm:3,fame:1}]}),d=>({text:ft("Irrlichter! Intelligenz!"),type:"skilltest",stat:"in_",diff:5+d,pass:{gold:10+roll(8),ruhm:5,fame:1},fail:{curse:true}}),d=>({text:ft("Skelettkrieger!"),type:"combat",enemy:mkE("Skelettarmee",d+2),reward:{gold:8+roll(8),ruhm:5,fame:1}}),d=>({text:ft("{adj} Truhe — Falle?"),type:"skilltest",stat:"ge",diff:5+d,pass:{gold:20+roll(10),ruhm:3},fail:{combat:mkE("Geisterwächter",d+2),reward:{gold:15,ruhm:5,fame:1}}})],
nebel:[d=>({text:ft("Schreie! Schiff von {cn} angegriffen!"),type:"choice",opts:["Helfen","Weiter"],rews:[{combat:mkE(ft("{cn}"),d+1),reward:{gold:10,ruhm:6,fame:1}},{}]}),d=>({text:ft("Nebel lichtet — {adj} Insel!"),type:"loot",gold:5+roll(8),ruhm:3,fame:1}),d=>({text:ft("Kompass wild! Navigator!"),type:"skilltest",stat:"in_",diff:5+d,pass:{ruhm:4,fame:1},fail:{ruhm:-2}}),d=>({text:ft("Sirenengesang!"),type:"skilltest",stat:"in_",diff:4+d,pass:{ruhm:3},fail:{combat:mkE("Sirenen",d+1),reward:{gold:8,ruhm:4,fame:1}}})],
festung:[d=>({text:ft("Kanonenbeschuss!"),type:"combat",enemy:mkE("Festungskanonen",d+3),reward:{gold:15+roll(10),ruhm:6,fame:1}}),d=>({text:ft("{adj} Geheimgang!"),type:"skilltest",stat:"ge",diff:6+d,pass:{gold:25+roll(15),ruhm:7,fame:2},fail:{combat:mkE("Festungswachen",d+2),reward:{gold:20,ruhm:6,fame:1}}}),d=>({text:ft("Gefangene Piraten!"),type:"choice",opts:["Befreien","Nein"],rews:[{combat:mkE("Garnison",d+2),reward:{ruhm:4,fame:2,gold:5}},{}]}),d=>({text:ft("Schatzkammer!"),type:"choice",opts:["Überfall!","Schleichen"],rews:[{combat:mkE("Elitegarde",d+3),reward:{gold:30+roll(20),ruhm:4,fame:2}},{gold:15+roll(10),ruhm:5,fame:1}]})],
vulkan:[d=>({text:ft("Lavaströme!"),type:"skilltest",stat:"ge",diff:5+d,pass:{ruhm:5,fame:1},fail:{dmgAll:2}}),d=>({text:ft("{adj} Drache bewacht {tn}!"),type:"combat",enemy:mkE("Vulkandrache",d+3),reward:{gold:20+roll(15),ruhm:4,fame:2}}),d=>({text:ft("Heisse Quellen!"),type:"heal",amount:4}),d=>({text:ft("Obsidianwaffen!"),type:"loot",gold:10+roll(8),ruhm:4,fame:1}),d=>({text:ft("Feuerelementar!"),type:"combat",enemy:mkE("Feuerelementar",d+3),reward:{gold:12+roll(10),ruhm:6,fame:1}})],
bermuda:[d=>({text:ft("Zeitverzerrung!"),type:"skilltest",stat:"in_",diff:6+d,pass:{ruhm:6,fame:1},fail:{curse:true}}),d=>({text:ft("Dimensionsriss!"),type:"choice",opts:["Hindurch!","Nein"],rews:[Math.random()>.4?{gold:30+roll(20),ruhm:4,fame:2}:{combat:mkE("Dimensionswächter",d+3),reward:{gold:20,ruhm:6,fame:1}},{ruhm:2}]}),d=>({text:ft("Strudel!"),type:"skilltest",stat:"st",diff:6+d,pass:{ruhm:5,fame:1},fail:{gold:-10}}),d=>({text:ft("{cn} aus anderer Dimension!"),type:"combat",enemy:mkE("Dimensionsbestie",d+3),reward:{gold:15+roll(10),ruhm:7,fame:1}})],
tiefsee:[d=>({text:ft("DER KRAKEN!!!"),type:"combat",enemy:mkE("Kraken",d+4),reward:{gold:20+roll(15),ruhm:5,fame:2}}),d=>({text:ft("Versunkene Stadt!"),type:"skilltest",stat:"ge",diff:6+d,pass:{gold:15+roll(15),ruhm:7,fame:1},fail:{dmgAll:2}}),d=>({text:ft("Leviathan!"),type:"combat",enemy:mkE("Leviathan",d+5),reward:{gold:25+roll(20),ruhm:6,fame:3}}),d=>({text:ft("Biolumineszenz!"),type:"loot",gold:3+roll(5),ruhm:4,fame:1})],
unterwasser:[d=>({text:ft("Davy Jones!"),type:"choice",opts:["Verhandeln","Kämpfen"],rews:[{ruhm:4,fame:2},{combat:mkE("Davy Jones",d+5),reward:{gold:30,ruhm:7,fame:4}}]}),d=>({text:ft("Jones Schatzkammer!"),type:"loot",gold:20+roll(20),ruhm:4,fame:2}),d=>({text:ft("{cn} in Jones Diensten!"),type:"combat",enemy:mkE("Jones Wächter",d+4),reward:{gold:15+roll(10),ruhm:4,fame:2}}),d=>({text:ft("{adj} Riesenperle — {tn}!"),type:"loot",gold:25+roll(15),ruhm:5,fame:2})],
schatz:[d=>({text:ft("X markiert die Stelle!"),type:"skilltest",stat:"in_",diff:5+d,pass:{gold:25+roll(20),ruhm:4,fame:2},fail:{combat:mkE("Schatzhueter",d+3),reward:{gold:30,ruhm:5,fame:2}}}),d=>({text:ft("{adj} Schatzkammer von {pn}!"),type:"loot",gold:20+roll(15),ruhm:4,fame:2}),d=>({text:ft("Pirat {pn} ist schon da!"),type:"combat",enemy:mkE(ft("Kpt. {pn}"),d+3),reward:{gold:20+roll(15),ruhm:6,fame:1}}),d=>({text:ft("Ein gewaltiger Schatzhueter bewacht die Kammer!"),type:"combat",enemy:mkE("Schatzkammer-Wächter",d+4),reward:{gold:30+roll(20),ruhm:6,fame:2}}),d=>({text:ft("Fallen! Geschick!"),type:"skilltest",stat:"ge",diff:6+d,pass:{gold:15+roll(10),ruhm:5,fame:1},fail:{dmgAll:3}})],
thron:[d=>({text:ft("Piratenrat! Duell um den Thron!"),type:"combat",enemy:mkE("Rivalen-Kapitäne",d+4),reward:{ruhm:7,fame:4,gold:20}}),d=>({text:ft("{pn} fordert zum Duell!"),type:"combat",enemy:mkE("Piratenkönig",d+5),reward:{ruhm:4,fame:5,gold:30}}),d=>({text:ft("Die Wächter des Throns greifen an!"),type:"combat",enemy:mkE("Thronwächter",d+5),reward:{ruhm:5,fame:3,gold:25}}),d=>({text:ft("Ein {adj} Prüfung der Weisheit!"),type:"skilltest",stat:"in_",diff:8+d,pass:{gold:20,ruhm:6,fame:3},fail:{gold:-10}}),d=>({text:ft("Sturmprüfung! Euer Schiff wird auf die Probe gestellt!"),type:"skilltest",stat:"ge",diff:8+d,pass:{ruhm:5,fame:2},fail:{dmgAll:3}}),d=>({text:ft("Alte Piratenlegenden flüstern euch Geheimnisse zu."),type:"loot",gold:10+roll(10),ruhm:5,fame:2}),d=>({text:ft("Ein {adj} Hinterhalt! Verräter in den eigenen Reihen!"),type:"combat",enemy:mkE("Verräter-Crew",d+4),reward:{ruhm:5,fame:2,gold:15}}),d=>({text:ft("Die Geister aller Piratenkönige prüfen eure Stärke!"),type:"skilltest",stat:"st",diff:9+d,pass:{ruhm:7,fame:3},fail:{curse:true}}),d=>{if(Math.random()<0.08)return{text:ft("Die {adj} Krone der Sieben Meere leuchtet auf dem Thron! IHR HABT SIE GEFUNDEN!"),type:"legendary"};return{text:ft("Der Thron ist nah — aber noch nicht bereit für euch. Kämpft weiter!"),type:"combat",enemy:mkE("Thronverteidiger",d+5),reward:{ruhm:5,fame:3,gold:20}};}],
verlies:[d=>({text:ft("Giftschlangen!"),type:"skilltest",stat:"ge",diff:5+d,pass:{gold:10+roll(8),ruhm:4,fame:1},fail:{combat:mkE("Riesenschlangen",d+2),reward:{gold:10,ruhm:4,fame:1}}}),d=>({text:ft("{adj} Schatztruhe — Fallen!"),type:"skilltest",stat:"ge",diff:4+d,pass:{gold:12+roll(10),ruhm:4,fame:1},fail:{dmgAll:2}}),d=>({text:ft("Skelette. Bei einem: {tn}!"),type:"loot",gold:8+roll(8),ruhm:5,fame:1}),d=>({text:ft("{cn} bewacht den Raum!"),type:"combat",enemy:mkE(ft("{cn}"),d+3),reward:{gold:15+roll(10),ruhm:6,fame:1}})],
hafen:[d=>({text:ft("Prügelei! {pn} beleidigt!"),type:"skilltest",stat:"st",diff:3+d,pass:{ruhm:2,gold:3},fail:{gold:-2}}),d=>({text:ft("Gerüchte: {tn} {loc}!"),type:"loot",gold:2+roll(5),ruhm:3,fame:1}),d=>({text:ft("Crew feiert! Rum fließt!"),type:"loot",gold:1+roll(3),ruhm:1,rum:3}),d=>({text:ft("Taschendieb!"),type:"skilltest",stat:"ge",diff:3+d,pass:{gold:5,ruhm:2},fail:{gold:-4}}),d=>({text:ft("{pn}: Geheimwissen für 8G."),type:"choice",opts:["Kaufen","Nein"],rews:[{gold:-8,ruhm:4,fame:1},{}]}),d=>({text:ft("Crew will an Land!"),type:"choice",opts:["Erlauben(-5G)","Nein"],rews:[{gold:-5,rum:5,ruhm:1},{ruhm:-1}]})],
dorf:[d=>({text:ft("Fischer: {cn} terrorisiert!"),type:"combat",enemy:mkE(ft("{cn}"),d),reward:{gold:5+roll(5),ruhm:3,fame:1}}),d=>({text:ft("Ältester kennt {adj} Schatz."),type:"loot",gold:2+roll(5),ruhm:3,fame:1}),d=>({text:ft("Frische Vorräte!"),type:"heal",amount:3}),d=>({text:ft("Kinder bewundern Crew!"),type:"loot",gold:1+roll(3),ruhm:2,rum:1})],
};
...
const checkPartyWipe=async(g,pi)=>{
  ...
  if(alive.length===0){
    ...
    g.log.push(`${g.players[pi].name}: ALLE K.O.! Rückzug nach ${REGIONS.find(r=>r.id===port)?.name}, -${lostGold}G`);
    setMsg(`Alle K.O.! Rückzug, -${lostGold} Gold.`);
    return true;
  }
  return false;
};

const moveTo=async rid=>{if(!isMyTurn)return;const r=REGIONS.find(x=>x.id===rid);if(me.fame<r.minF){setMsg(`${r.minF}⭐ nötig!`);return;}
  const g={...game};const pi=g.players.findIndex(p=>p.id===playerId);g.players[pi]={...g.players[pi],position:rid};
  g.log.push(`${me.name}: Segelt nach ${r.name}`);
  setMsg(`Angekommen: ${r.name} ${REMO[r.type]}`);
  await endTurn(g);}; // Movement ENDS the turn — explore next turn!
...
const learnSkill=async(hid,skill)=>{if(me.ruhm<10)return;const g={...game};const pi=g.players.findIndex(p=>p.id===playerId);g.players[pi].ruhm-=10;
  const h=g.players[pi].heroes.find(x=>x.id===hid);if(h){h.skills=[...(h.skills||[]),skill];
    ...
    if(["Feilschen","Kräuterkunde","Giftkunde","Geisterruf"].includes(skill))h.in_+=1;
    if(["Breitseite","Kettenschuss","Präzisionsschuss"].includes(skill))h.ge+=1;
...
const healHero=async(hid)=>{if(me.gold<healCost)return;const g={...game};const pi=g.players.findIndex(p=>p.id===playerId);
  ...
  await api.save(g);setGame(g);};
const recruitHero=async()=>{if(!rName.trim()||!rRace||!rProf){setMsg("Alles ausfüllen!");return;}if(me.gold<recruitCost){setMsg("Kein Gold!");return;}
  const g={...game};const pi=g.players.findIndex(p=>p.id===playerId);
    ...
    :<div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:10}}>
        {heroes.filter(Boolean).map((h,i)=>(<div key={h.id} onClick={()=>setSelHero(i)}
          ...
          {(h.equipment||[]).length===0&&<div style={{fontSize:9,color:T.red,marginTop:2}}>Keine Ausrüstung</div>}
        </div>))}
      </div>
...
const PlayScreen=()=>{const other=game?.players?.find(p=>p.id!==playerId);
  return <div style={{padding:14,paddingBottom:100}}>
    ...
    {showOpp&&other&&<Card style={{background:T.bg,marginBottom:8}}>
      ...
      {(other.curses||[]).length>0&&<div style={{fontSize:8,color:T.red,marginTop:4}}>Flüche: {other.curses.map(c=>c.name).join(", ")}</div>}
...
const EventScreen=()=>(<div style={{minHeight:"100vh",padding:20,display:"flex",flexDirection:"column",justifyContent:"center"}}>
  <Card style={{borderColor:T.gold+"44"}}>
    ...
    {ev?.type==="skilltest"&&!testRes&&(<div><div style={{textAlign:"center",fontSize:12,color:T.gold,marginBottom:8}}>{ev.stat==="st"?"Stärke":ev.stat==="ge"?"Geschick":"Intelligenz"}-Test (≥{ev.diff})</div>
      <Btn primary onClick={()=>resolveEvent()}>Würfeln!</Btn></div>)}
    ...
    {ev?.type==="legendary"&&<div><div style={{textAlign:"center",fontSize:28,marginBottom:10}}>👑💎</div><Btn primary onClick={()=>resolveEvent()}>PIRATENKÖNIG!</Btn></div>}
...
const FinishedScreen=()=>(<div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
  ...
  <div style={{fontSize:24,fontWeight:900,color:T.gold,fontFamily:"'Cinzel',serif",marginBottom:4}}>{game?.winner?.type==="legendary"?"LEGENDÄR!":"PIRATENKÖNIG!"}</div>
...
return(<div style={{background:T.bg,minHeight:"100vh",color:T.txt,fontFamily:"'Crimson Text',serif",maxWidth:600,margin:"0 auto"}}>
  ...
  {showHelp&&<div onClick={()=>setShowHelp(false)} style={{position:"fixed",inset:0,background:"#000c",zIndex:998,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div onClick={e=>e.stopPropagation()} style={{background:T.card,border:`1px solid ${T.gold}`,borderRadius:16,padding:20,maxWidth:380,maxHeight:"85vh",overflow:"auto",width:"100%"}}>
      ...
      {[
        ["Ziel","Sammle 100 Ruhm-Sterne (⭐) oder finde den legendären Schatz auf der Schatzinsel, um Piratenkönig zu werden!"],
        ["Crew","Deine 4 Piraten haben je 4 Werte: ST (Stärke), GE (Geschick), IN (Intelligenz), BW (Bewegung). NK = Nahkampf (ST + Beruf + Waffen). RW = Rüstungswert."],
        ["Zuege","Pro Zug: Erkunden (W100-Ereignis), Rasten (heilt HP, kostet Rum), Laden, Hafen oder Aufwerten. Dann ist der Gegner dran."],
        ["Erkunden","Würfelt ein zufaelliges Ereignis: Kaempfe, Fertigkeitstests (W6 + Stat vs Schwierigkeit), Entscheidungen, Schaetze, Flüche oder Handel."],
        ["Kampf","Jeder lebende Held würfelt W6 + NK. Auf See: Schiffskanonen addieren sich. Gegner würfelt 2W6 + NK. Hoehere Summe trifft — Schaden = Differenz minus RW. Trifft den schwaechsten Helden."],
        ["Schiffe","Kanonen = Angriffsbonus auf See. Rumpf = Rüstungsbonus auf See. Kaufbar in Haefen mit Schiffsverkauf."],
        ["Hafen","K.O.-Helden heilen oder neue Soeldner anheuern (ersetzt K.O.-Helden). Alle K.O. = automatischer Rückzug zum naechsten Hafen, -25% Gold."],
        ...
        ["Flüche","Manche Events verfluchen euch — Stat-Mali, Gold halbiert oder Extra-Rumkosten. Entfernbar bei Voodoo-Priesterinnen im Sumpf."],
        ["Speichern","Spiele speichern automatisch. Menue zeigt alle gespeicherten Spiele zum Fortsetzen — auch Tage spaeter. Mehrere Spiele parallel möglich."],
